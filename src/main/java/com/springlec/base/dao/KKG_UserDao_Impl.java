package com.springlec.base.dao;

import java.util.List;

import org.apache.ibatis.session.SqlSession;

import com.springlec.base.model.AdminExtra_Dto_kkg;

public class KKG_UserDao_Impl implements KKG_UserDao {

	SqlSession sqlSession;
	public String nameSpace = "com.springlec.base.dao.KKG_UserDao";
	
	

	@Override
	public List<AdminExtra_Dto_kkg> getUserList(int pageNum) throws Exception {
		// TODO Auto-generated method stub
		return sqlSession.selectList(nameSpace+".getUserList");
	}



	@Override
	public List<AdminExtra_Dto_kkg> getUserCount() throws Exception {
		// TODO Auto-generated method stub
		return sqlSession.selectList(nameSpace+".getUserCount");
	}

	
	
	
}
